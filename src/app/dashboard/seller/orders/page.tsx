...
export default function SellerOrdersPage() {
  ...
  const [activeOrderItem, setActiveOrderItem] =
  ...
  const [viewDeliveries, setViewDeliveries] =
    useState<OrderItemDelivery[] | null>(null);
  ...
  function getStatusBadge(status: string) {
    switch (status) {
      case "pending":
        return { label: "Pending", icon: <Clock size={14} />, variant: "warning" as const };
      case "in_escrow":
        return { label: "In Escrow", icon: <CheckCircle size={14} />, variant: "success" as const };
      case "delivered":
        return { label: "Delivered", icon: <Truck size={14} />, variant: "outline" as const };
      case "completed":
        return { label: "Completed", icon: <CheckCircle size={14} />, variant: "default" as const };
      case "cancelled":
      case "rejected":
        return { label: status, icon: <XCircle size={14} />, variant: "destructive" as const };
      case "disputed":
        return { label: "Disputed", icon: <AlertCircle size={14} />, variant: "destructive" as const };
      default:
        return { label: status, icon: null, variant: "outline" as const };
    }
  }
...
async function handleSubmitDelivery() {
  ...
  async function handleViewDeliveries(orderItem: OrderItemType) {
    if (!access) return;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
          ...
          <Card key={order.id}>
            <CardHeader>
              <CardTitle className="flex justify-between">
                ...
                <Badge variant={badge.variant} className="flex gap-1 items-center">
                  {badge.icon}
                  {badge.label}
                </Badge>
              </CardTitle>
                ...
                <div key={item.order_item_id} className="border rounded p-4 space-y-2">
                 <div className="font-medium">{item.marketplace_item.title}</div>
                  ...
                  {order.status === "in_escrow" && (
                    <Button
                      size="sm"
                      onClick={() => setActiveOrderItem(item)}
                    >
                      Submit Delivery
                    </Button>
                  )}

                  {(item.delivered_quantity > 0 || order.status === "delivered") && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleViewDeliveries(item)}
                    >
                      View Deliveries
                    </Button>
                  )}
      ...
      ======================= */}
      <Dialog open={!!activeOrderItem} onOpenChange={() => setActiveOrderItem(null)}>
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setActiveOrderItem(null)}>
              Cancel
            </Button>
            <Button onClick={handleSubmitDelivery}>Submit</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <div className="space-y-4">
            {viewDeliveries?.map((d) => (
              <div key={d.id} className="border rounded p-3 text-sm space-y-1">
                ...
                {d.repo_url && (
                  <a href={d.repo_url} target="_blank" className="text-blue-500">
                    {d.repo_url}
                  </a>
                )}
                {d.file && (
                  <a href={d.file} target="_blank" className="text-blue-500">
                    Download File
                  </a>
                )}
