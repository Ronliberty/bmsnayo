...
export default function SellerOrdersPage() {
  ...
  useEffect(()=> {
    ...
    async function fetchOrders() {
      try {
        const r = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/market/orders/?role=seller`, {
      ...
      <div className="space-y-4">
        {orders.map(order => (
          <Card key={order.id}>
            <CardHeader>
              <CardTitle>{order.item.title}</CardTitle>
              <div className="text-xs text-muted-foreground">Order #{order.id} • {order.buyer_name || order.buyer}</div>
            </CardHeader>
            <CardContent className="space-y-2">
              <p><strong>Status:</strong> {order.status}</p>
              <p><strong>Quantity:</strong> {order.quantity}</p>
              <p><strong>Total:</strong> {order.item.currency} {order.item.price}</p>
              <p><strong>Delivery Type:</strong> {order.item.delivery_type ?? 'manual'}</p>

              <div className="mt-2">
                <Link href={`/dashboard/marketplace/orders/${order.id}`}>
